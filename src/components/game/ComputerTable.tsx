...
export const ComputerTable = ({ gameState, selectedCardIndex, onSelectCard, onDraw, onDiscard, onClaim, onRematch, cardMotion, turnPaused }: ComputerTableProps) => {
  ...
  const canClaim = !gameState.gameEnded && gameState.canClaim && !!gameState.lastDiscardedCard &&
    canUseCardToWin(human?.hand ?? [], gameState.lastDiscardedCard);
    ...
    <main className="relative mx-auto flex min-h-[calc(100vh-2rem)] max-w-[1500px] items-center justify-center overflow-hidden px-2 pb-4 pt-20 sm:px-8 sm:pt-16">
      <div className="absolute left-1/2 top-5 z-30 -translate-x-1/2 text-center">
        ...
        <p className="mt-1 text-sm font-semibold text-foreground/80">
          {gameState.gameEnded ? `${gameState.winner?.name ?? 'You'} wins!` : turnPaused ? `${currentPlayer?.name ?? ''} is considering the discard…` : currentPlayer?.isWinner ? 'Finishing round…' : `${currentPlayer?.name ?? ''}'s turn`}
        </p>
        {gameState.gameEnded && (
      ...
      <section className="njuka-table relative h-[68vh] min-h-[470px] w-full max-w-[1180px]">
        {cardMotion && (
          <div key={cardMotion.key} className={cn(
            'pointer-events-none absolute left-1/2 top-[43%] z-50',
            cardMotion.target === 'human'
              ? (cardMotion.kind === 'draw' ? 'njuka-card-draw' : 'njuka-card-discard')
              : (cardMotion.kind === 'draw' ? 'njuka-card-draw-ai' : 'njuka-card-discard-ai')
          )}>
        ...
        <div className="absolute bottom-[-5%] left-1/2 z-30 w-full max-w-xl -translate-x-1/2 text-center">
          <div className={cn('mx-auto mb-3 inline-flex rounded-full border bg-background/90 px-5 py-2 font-bold shadow-xl',
            ...
            You {human?.isCurrentTurn && !gameState.gameEnded ? '· Your turn' : ''}
          </div>
          ...
          {canDiscard && (
            <Button onClick={() => selectedCardIndex !== null && onDiscard(selectedCardIndex)} disabled={selectedCardIndex === null} className="mt-2 bg-gradient-gold text-background">
